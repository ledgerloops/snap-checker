...
Ledger.prototype = {
  ...
  _startLoop: function(routeId, fsidePeer) {
    ...
    const fsideBal = this._handler._agent._peerHandlers[fsidePeer].getBalance(); // our fside balance should be low because it will go up
    ...
    this._handler._agent._preimages[hashHex] = preimage;
    if (amount <0) {
  ...
  _createProbe: function() {
    ...
    for(let fsideLedger in this._handler._agent._peerHandlers) {
      // Our fside balance will go down, so find one whose balance is higher:
      if (this._handler._agent._peerHandlers[fsideLedger].getBalance() > thisBal) {
        this._handler._agent._peerHandlers[fsideLedger].send({
          msgType: 'PROBES',
          cwise: [],
          fwise: [ newProbe ]
        });
  ...
  _handleProbe: function(msg) {
    ...
    const thisBal = this.getBalance();
    for(let otherPeer in this._handler._agent._peerHandlers) {
      this._handler._agent._peerHandlers[otherPeer].considerProbe(thisBal, msg, this._peerNick);
    }
  },
